    ...
    def run_purchase():
        with sync_playwright() as p:
            ...
            try:
                ...
                logging.warning("Yeni selector'lar ile işlem yapılamadı, eski selector'lara geçiliyor.")
                try:
                    page.wait_for_selector(".action-select-btn:visible", timeout=30000)
                    page.click(".action-select-btn")
                    page.wait_for_selector("[data-testid='providerSelectBtn']:visible", timeout=30000)
                    page.click("[data-testid='providerSelectBtn']")
                    final_url = page.url
                except PlaywrightTimeoutError:
                    logging.error("Her iki selector denemesinde de hata oluştu, mevcut sayfa URL'si kullanılıyor.")
                    final_url = page.url
