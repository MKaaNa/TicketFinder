document.getElementById("fetchFlights").addEventListener("click", async function() {
    ...
    try {
        ...
        let enuygunFlights = data.filter(flight => flight.kaynak === "Enuygun")
            .map(flight => `<li>ID: ${flight.flight_id || "Bilinmiyor"} - ${flight.airline || "Bilinmiyor"} / ${flight.promo || "Bilinmiyor"} / ${flight.departure_time || "Bilinmiyor"} / ${flight.price || "Bilinmiyor"}</li>`)
            .join("");
        let obiletFlights = data.filter(flight => flight.kaynak === "Obilet")
            .map(flight => `<li>ID: ${flight.flight_id || "Bilinmiyor"} - ${flight.airline || "Bilinmiyor"} - ${flight.price || "Bilinmiyor"} - ${flight.departure_time || "Bilinmiyor"}</li>`)
            .join("");
        let turnaFlights = data.filter(flight => flight.kaynak === "Turna")
            .map(flight => `<li>${flight.summary || "Bilinmiyor"}</li>`)
            .join("");
        flightResults.innerHTML = `
